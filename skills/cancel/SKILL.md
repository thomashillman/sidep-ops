---
name: cancel
description: Cancel any active OMC mode (autopilot, ralph, ultrawork, qa, swarm, ultrapilot, pipeline, team)
level: 2
---
...
1. Autopilot (includes linked ralph/qa/ cleanup)
...
Autopilot handles its own cleanup including linked ralph and qa.
...
3. Check for linked qa via `state_read(mode="qa", session_id)`:
   - If active, clear it: `state_clear(mode="qa", session_id)`
...
Clear directly: `state_clear(mode="qa", session_id)`
