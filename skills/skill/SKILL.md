...
Report findings and ask if user wants to extract any as skills (invoke `/learn` if yes).
...
- `/skill add` - Start skill creation (invoke learn)
...
- `/learn` - Extract a skill from current conversation
- `/note` - Save quick notes (less formal than skills)
- `/init` - Generate AGENTS.md codebase hierarchy
...
- After solving a tricky bug, immediately run learn to capture it
