...
All data persists in `.note/memory.json` for git-friendly collaboration.

## Commands

| Command | Action |
|---------|--------|
| `/note init <project-name>` | Initialize new project memory |
| `/note status` | Show memory overview (character count, scene count, etc) |
| `/note char add <name>` | Add new character |
| `/note char <name>` | View character details |
| `/note char update <name> <field> <value>` | Update character field |
| `/note char list` | List all characters |
| `/note rel add <char1> <char2> <type>` | Add relationship |
| `/note rel <char1> <char2>` | View relationship |
| `/note rel update <char1> <char2> <event>` | Add relationship event |
| `/note scene add <title>` | Add new scene |
| `/note scene <id>` | View scene details |
| `/note scene list` | List all scenes |
| `/note theme add <name>` | Add theme |
| `/note world set <field> <value>` | Set world attribute |
| `/note query <question>` | Query memory naturally (Korean supported) |
| `/note validate <character> <dialogue>` | Check if dialogue matches character tone |
| `/note synopsis` | Generate emotion-focused synopsis |
| `/note export` | Export full memory as readable markdown |
| `/note backup` | Create manual backup |
...
```
/note char add 새랑
/note char update 새랑 arc "체념 -> 욕망자각 -> 선택"
/note char update 새랑 tone "담백, 현재충실, 감정억제"
/note char update 새랑 speechLevel "해체"
/note char update 새랑 keywords "그냥, 뭐, 괜찮아"
/note char update 새랑 taboo "사랑해, 보고싶어"
```
...
**Example:**
```
/note rel add 새랑 해랑 romantic
/note rel update 새랑 해랑 "첫 키스 - 새랑 회피"
/note rel update 새랑 해랑 "해랑 고백 거절당함"
/note rel update 새랑 해랑 "새랑 먼저 손 잡음"
```
...
**Example:**
```
/note validate 새랑 "사랑해, 해랑아. 너무 보고싶었어."
```
Output:
```
...
```
/note query "새랑은 이 상황에서 뭐라고 할까?"
/note query "규리의 현재 감정 상태는?"
/note query "해랑과 새랑의 관계는 어디까지 왔나?"
/note query "이 장면의 정서적 분위기는?"
/note query "새랑이 먼저 연락하는 게 맞아?"
/note query "해랑이 화났을 때 말투는?"
```
...
1. **On Init**: Creates `.note/memory.json` with project metadata and empty collections
2. **Auto-Backup**: Changes are backed up before modification to `.note/backups/`
...
/note char add 새랑
/note char update 새랑 arc "체념 -> 욕망자각 -> 선택"
/note char update 새랑 tone "담백, 현재충실"
/note char update 새랑 speechLevel "해체"

/note char add 해랑
/note char update 해랑 arc "확신 -> 동요 -> 기다림"
/note char update 해랑 tone "직진, 솔직"
/note char update 해랑 speechLevel "반말"

# Establish relationship
/note rel add 새랑 해랑 romantic
/note rel update 새랑 해랑 "첫 만남 - 해랑 일방적 호감"
/note rel update 새랑 해랑 "새랑 거절"
/note rel update 새랑 해랑 "재회 - 새랑 내적 동요"

# Set world
/note world set setting "서울, 현대, 20대 후반 직장인"
/note world set atmosphere "도시의 건조함 속 미묘한 온기"

# Add themes
/note theme add "포기하지 않는 사랑"
/note theme add "자기 보호의 벽"

# Add scene
/note scene add "옥상 재회"

# Query for writing
/note query "새랑은 이별 장면에서 어떤 톤으로 말할까?"

# Validate dialogue
/note validate 새랑 "해랑아, 그만하자."
...
**Memory not loading?**
- Check `.note/memory.json` exists
- Verify JSON syntax is valid
- Run `/note status` to diagnose
