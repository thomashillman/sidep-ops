...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   - Remove `.omc/state/autopilot-state.json`, `ralph-state.json`, `ultrawork-state.json`, `qa-state.json`
   - Run `/cancel` for clean exit
</Steps>

<Tool_Usage>
- Use `Task(subagent_type="architect", ...)` for Phase 4 architecture validation
- Use `Task(subagent_type="security-reviewer", ...)` for Phase 4 security review
- Use `Task(subagent_type="code-reviewer", ...)` for Phase 4 quality review
