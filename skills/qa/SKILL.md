...
|------------|-----------|---------------|
| `/qa --tests` | tests | All test suites pass |
| `/qa --build` | build | Build succeeds with exit 0 |
| `/qa --lint` | lint | No lint errors |
| `/qa --typecheck` | typecheck | No TypeScript errors |
| `/qa --custom "pattern"` | custom | Custom success pattern in output |
...
Track state in `.omc/qa-state.json`:
```json
{
  "active": true,
  "goal_type": "tests",
...
```bash
# Delete qa state file
rm -f .omc/state/qa-state.json
```
