...
```
/science <goal>                    # Standard research with user checkpoints
/science AUTO: <goal>              # Fully autonomous until complete
/science status                    # Check current research session status
/science resume                    # Resume interrupted research session
/science list                      # List all research sessions
/science report <session-id>       # Generate report for session
```

### Quick Examples

```
/science What are the performance characteristics of different sorting algorithms?
/science AUTO: Analyze authentication patterns in this codebase
/science How does the error handling work across the API layer?
```
...
Task(subagent_type="scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
```
/science AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/science status` | Show current session progress |
| `/science resume` | Resume most recent interrupted session |
| `/science resume <session-id>` | Resume specific session |
| `/science list` | List all sessions with status |
| `/science report <session-id>` | Generate/regenerate report |
| `/science cancel` | Cancel current session (preserves state) |
