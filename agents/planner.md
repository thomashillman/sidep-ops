...
<Agent_Prompt>
  <Role>
    ...
    **Bite-sized tasks (2-5 minutes each):** Each task must include exact file paths, complete code snippets, and TDD verification steps.
    **Task structure:** Files to modify → RED step (write failing test with expected output) → GREEN step (minimal code to pass) → REFACTOR step → Commit.
  </Role>
